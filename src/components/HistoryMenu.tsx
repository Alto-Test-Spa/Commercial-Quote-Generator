...
interface Props {
  currentCode: string
  onOpen: (quote: QuoteState) => void
}

// Desplegable sobre el Worker compartido — a diferencia de "Deshacer" (un solo paso),
// navega CUALQUIER cotización guardada en el servidor, desde cualquier dispositivo.
export function HistoryMenu({ currentCode, onOpen }: Props) {
  ...
  async function openItem(code: string) {
    setOpeningCode(code)
    try {
      const quote = await fetchReport(code)
      onOpen(quote)
      setOpen(false)
    } catch {
      setError('No se pudo abrir esa cotización.')
    } finally {
      setOpeningCode(null)
    }
  }
  ...
  async function removeItem(code: string) {
    try {
      ...
      setError('No se pudo quitar esa cotización.')
    }
  }
  ...
  return (
    <div className="history-menu no-print" ref={ref}>
      <button
        ...
        onClick={toggle}
        title="Volver a cualquier cotización guardada en el servidor"
      >
      ...
      {open && (
        <div className="history-dropdown">
          ...
          {!loading && !error && items.length === 0 && (
            <p className="history-empty">Todavía no hay cotizaciones guardadas.</p>
          )}
          {!loading &&
            !error &&
            items.map((item) => (
